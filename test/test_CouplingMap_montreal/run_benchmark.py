...
with open(resultfile, 'w') as csvfile:
    ...
    result = Result(circuit, basis_gates=basis_gates, coupling_map=coupling_map, routing_method="sabre")
    result.run_pms(passmanagers, times=times)
    writer.writerow(result.row(fields))

    result = Result(circuit, basis_gates, coupling_map, routing_method="NASSCSwap")
    result.run_pms(passmanagers, times=times)
    writer.writerow(result.row(fields))
