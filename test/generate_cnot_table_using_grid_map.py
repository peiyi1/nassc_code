...
csv_path_CouplingMap_grid=os.path.dirname(os.path.abspath(__file__))+"/test_CouplingMap_grid/results/"
...
def obtain_data_fully_connected_map(csv_file):
    with open(csv_path_CouplingMap_FullyConnected +csv_file, newline='') as f:
        ...
        return dict_data
def obtain_data_grid_map(csv_file):
    with open(csv_path_CouplingMap_grid+csv_file, newline='') as f:
        reader = csv.reader(f)
        headings = next(reader)
        #get the result of sabre
        first_row= next(reader)
...
for csv_file in csv_file_dict_grid_map:
    csv_file_dict_grid_map[csv_file]=obtain_data_grid_map(csv_file+'.csv')
'''
...
with open('cnot_table_using_grid_map.csv', 'w') as csvfile:
    ...
    for csv_file in labels:
        ...
        CNOT_total_Qiskit_SABRE = csv_file_dict_grid_map[csv_file][0]['num_cnot']
        CNOT_total_Qiskit_NASSC = csv_file_dict_grid_map[csv_file][1]['num_cnot']
        ...
        transpile_time_Qiskit_SABRE = csv_file_dict_grid_map[csv_file][0]['time']
        transpile_time_Qiskit_NASSC = csv_file_dict_grid_map[csv_file][1]['time']
