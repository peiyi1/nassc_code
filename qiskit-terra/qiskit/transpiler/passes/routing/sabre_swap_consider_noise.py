...
class SabreSwapConsiderNoise(TransformationPass):
    ...
    **References:**
    ...
    def __init__(self, coupling_map, heuristic="basic", seed=None, fake_run=False,  hardware=None):
    ...
    def run(self, dag):
        ...
        while front_layer:
            ...
            for swap_qubits in list(swap_scores):
                trial_layout = current_layout.copy()
                score_before_swap = self._score_heuristic(
                    self.heuristic, front_layer, extended_set, trial_layout, swap_qubits
                )
                trial_layout.swap(*swap_qubits)
                score_after_swap = self._score_heuristic(
                    self.heuristic, front_layer, extended_set, trial_layout, swap_qubits
                )
                if score_after_swap - score_before_swap > 0 :
                    del swap_scores[swap_qubits]
                else:
                    score = self._score_heuristic_consider_noise(
                        self.heuristic, front_layer, extended_set, trial_layout, swap_qubits
                    )
